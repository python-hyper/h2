...
from hpack.struct import HeaderTuple, NeverIndexedHeaderTuple, Headers
...
from .events import (
    Event,
    AlternativeServiceAvailable, ConnectionTerminated, DataReceived,
    InformationalResponseReceived,
    PingAckReceived, PingReceived, PriorityUpdated, PushedStreamReceived,
    RemoteSettingsChanged, RequestReceived, ResponseReceived,
    SettingsAcknowledged, StreamEnded, StreamReset,
...
class H2Connection:
    ...
    def send_data(self, stream_id: int, data: Union[bytes, memoryview], end_stream: bool = False, pad_length: Any = None) -> None:
        """
    ...
    def _receive_headers_frame(self, frame: HeadersFrame) -> tuple[list[Frame], list[Event]]:
        ...
        if 'PRIORITY' in frame.flags:
            ...
            assert isinstance(stream_events[0], (RequestReceived, ResponseReceived, TrailersReceived, InformationalResponseReceived))
    ...
    def _handle_data_on_closed_stream(self, events: list[Event], exc: StreamClosedError, frame: DataFrame) -> tuple[list[Frame], list[Event]]:
...
def _decode_headers(decoder: Decoder, encoded_header_block: bytes) -> Headers:
    """
